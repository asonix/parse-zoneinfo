...
impl LineParser {
    pub fn new() -> Self {
        LineParser {
            rule_line: Regex::new(r##"(?x)^Rule\s+(?P<name>\S+)\s+(?P<from>\S+)\s+(?P<to>\S+)\s+(?P<type>\S+)\s+(?P<in>\S+)\s+(?P<on>\S+)\s+(?P<at>\S+)\s+(?P<save>\S+)\s+(?P<letters>\S+)\s*(\#.*)?$"##).unwrap(),

            day_field: Regex::new(r##"(?x)^(?P<weekday>\w+)(?P<sign>[<>]=)(?P<day>\d+)$"##).unwrap(),

            hm_field: Regex::new(r##"(?x)^(?P<sign>-?)(?P<hour>\d{1,2}):(?P<minute>\d{2})(?P<flag>[wsugz])?$"##).unwrap(),

            hms_field: Regex::new(r##"(?x)^(?P<sign>-?)(?P<hour>\d{1,2}):(?P<minute>\d{2}):(?P<second>\d{2})(?P<flag>[wsugz])?$"##).unwrap(),

            zone_line: Regex::new(r##"(?x)^Zone\s+(?P<name>[A-Za-z0-9/_+-]+)\s+(?P<gmtoff>\S+)\s+(?P<rulessave>\S+)\s+(?P<format>\S+)\s*(?P<year>\S+)?\s*(?P<month>\S+)?\s*(?P<day>\S+)?\s*(?P<time>\S+)?\s*(\#.*)?$"##).unwrap(),

            continuation_line: Regex::new(r##"(?x)^\s+(?P<gmtoff>\S+)\s+(?P<rulessave>\S+)\s+(?P<format>\S+)\s*(?P<year>\S+)?\s*(?P<month>\S+)?\s*(?P<day>\S+)?\s*(?P<time>\S+)?\s*(\#.*)?$"##).unwrap(),

            link_line: Regex::new(r##"(?x)^Link\s+(?P<target>\S+)\s+(?P<name>\S+)\s*(\#.*)?$"##).unwrap(),

            empty_line: Regex::new(r##"(?x)^\s*(\#.*)?$"##).unwrap(),
        }
    }
}
...
impl ChangeTime {
    pub fn to_timestamp(&self) -> i64 {
        ...
        fn time_to_timestamp(
            ...
            seconds_until_start_of_year(year) + 60 * 60 * 24 * if is_leap(year) {
                MONTHS_LEAP[month as usize - 1]
            } else {
                MONTHS_NON_LEAP[month as usize - 1]
            } + 60 * 60 * 24 * (day as i64 - 1) + 60 * 60 * hour as i64
                + 60 * minute as i64 + second as i64
        }
